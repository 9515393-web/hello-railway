...
async def about_cmd(message: types.Message):
    await message.answer(
        ...
        "⬇️ Текст начинается ниже ⬇️\n"
        "Прокрутите чат вверх, чтобы читать с начала."
    )
    parts = [
        "🏡 **Зачем восстанавливать деревню Захожье?**\n"
        "*Кратко и по делу*\n\n"
...
async def map_1792_cmd(message: types.Message):
    await message.answer_photo(
        ...
        caption="🗺 **Выкопировка из карты Санкт-Петербургской губернии, 1792 г.**\n\n"
...
@dp.message(F.text == "🗺 План деревни 1885 г.")
async def map_1885_cmd(message: types.Message):
    await message.answer_photo(
        FSInputFile("maps/map_1885.jpg"),
        caption="🗺 **План деревни Захожье, 1885 год**\n\n"
...
async def history_cmd(message: types.Message):
    await message.answer(
        ...
        "⬇️ Текст начинается ниже ⬇️\n"
        "Прокрутите чат вверх, чтобы читать историю с начала."
    )
    parts = [
...
async def roadmap_cmd(message: types.Message):
    await message.answer(
        ...
        "⬇️ План начинается ниже ⬇️\n"
        "Прокрутите чат вверх, чтобы читать с первого этапа."
    )
    parts = [
        "🗺 **ДОРОЖНАЯ КАРТА восстановления деревни Захожье**\n"
...
# ===== ПРОЧЕЕ =====
@dp.message(F.text == "🗳 Принять участие")
async def vote_cmd(message: types.Message):
    uid = message.from_user.id
    if await has_voted(uid):
        await message.answer("Вы уже участвовали.")
        return
    await register_vote(uid)
    await message.answer(f"👉 {GOOGLE_FORM_URL}")
